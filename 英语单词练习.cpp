...
struct Word{
	...
	bool operator==(const Word &o){
		return this->mean==o.mean&&this->wd==o.wd;
	}
	bool operator<(const Word &o){
		return this->mean<o.mean||this->wd<o.wd;
	}
...
void init(){
	...
	while(getline(cin,t)){
		...
		for(int i=t.size();i;i--){
			if(t[i]==' '){
				if(f==0){
					reference.back().mean=t.substr(i);
					reference.back().wd=t.substr(t.find(" "),i-t.find(" "));
					break;
				}
			}
		}
...
int all=0,correct=0;
void show(){
	cout<<"all: "<<all<<"   correct: "<<correct<<"    ---";
	if(all)cout<<100.0*correct/all;
	else cout<<0;
	cout<<"%\n";
}
void AC(){
...
//	}
	while(1){
		...
		int ans;
		while(key.size()<4){
			ans=(Rad()%((int)reference.size()));
			key.insert(ans);
		}
		vector<int>Keys;
		for(auto &i:key)Keys.push_back(i);
		shuffle(Keys.begin(),Keys.end(),default_random_engine(time(0)));
		cout<<reference[ans].wd<<"\n";
		int tans=0;
		for(int i=0;i<4;i++){
			if(Keys[i]==ans)tans=i;
			cout<<i+1<<" "<<reference[Keys[i]].mean<<"\n";
		}
		int t=getch();
		if(t-49==tans)AC();
		else WA();
